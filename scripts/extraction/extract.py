...
class StoryId:
    ...
    def parseFromPath(cls, text_type: str, path: str):
        if text_type == "home":
            ...
            # Extract id from path like '.../m9051/m9051_lyrics'
    ...
    def load(self):
        ...
        for obj in objects:
            try:
                ...
                # Check for standard story blocks or Lyrics script (TextAsset)
                if "BlockList" in tree or "TextTrack" in tree or "m_Script" in tree:
                    self.rootAsset = obj
                    break
            except Exception:
                continue
...
class Args(argparse.ArgumentParser):
    def __init__(self, desc, **kwargs) -> None:
        ...
        self.add_argument("-dst", type=Path, default=Path("data/raw"), help="Output directory")
...
def extractAsset(asset: GameBundle, storyId: StoryId, current_args) -> Union[None, str]:
    ...
    # =========================================================
    # SPECIAL HANDLING FOR LYRICS (CSV parsing -> Flat Dict)
    # =========================================================
    if current_args.type == "lyrics":
        if "m_Script" not in tree:
            return None
        
        lyrics_export = {}
        # Parse CSV from m_Script
        # Format expects keys to be time (string), values to be text
        try:
            ...
            for row in r:
                ...
                # row[0] = time, row[1] = text
                lyrics_export[row[0]] = row[1]
                
        except Exception as e:
            print(f"Error parsing lyrics csv for {storyId.id}: {e}")
            return None

        if not lyrics_export:
            return None

        filename = f"{storyId.getFilenameIdx()}.json"
        return lyrics_export, filename

    # =========================================================
    # STANDARD HANDLING (Story / Home / Preview)
    # =========================================================
    export = {
